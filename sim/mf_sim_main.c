/**
 * @file mf_sim_main.c
 * @brief MultiFOCAL's own interactive host simulator entry point - the
 *        real, hand-usable counterpart to test/'s automated keystroke-
 *        injection tests. Adapted from soynut's own sim/sim_main.c
 *        (copied, not modified in place, per this project's standing
 *        "nothing under ~/soynut is built or modified" convention - see
 *        CLAUDE.md's "Relationship to ~/soynut" section) with exactly
 *        one addition: after the same `nut_boot()` every soynut sim run
 *        does, this also wires in the real, user-sourced HP 82180A
 *        Extended Functions/Memory module ROM (page 5, Port 2) and
 *        MultiFOCAL's own `frames.mod` (page 6, Port 3) - the identical
 *        CV+82180A boot configuration already verified correct by
 *        test/cv82180a_*_test.c, just driven by a real SDL window and a
 *        real keyboard instead of programmatic keystroke injection.
 *
 * Every other line of logic (CLRMEM handling, Elite Mode toggles, the
 * asleep/wake transition, the hold-vs-batch executeNUT() split, the
 * fdsp-gated render, POWOFF's dspon-conditional clear, deferred
 * continuous-memory save) is soynut's own, completely unchanged - see
 * that file's own header for the full rationale. `nut_boot()` itself
 * only ever touches pages 0-2 (confirmed in firmware/emu41gcc_compat/
 * nut_rom.c), so the module wiring below survives a CLRMEM-triggered
 * re-boot with no extra handling needed.
 */
...
/* build/frames_rom.c (generated from src/frames.s - see the top-level
 * Makefile's `mod` target) has no header of its own, same pattern the
 * test/ suite already uses for every generated ROM array. */
...
int main(void)
{
    ...
    /* MultiFOCAL's own addition: wire in the real 82180A module (page
     * 5, Port 2) and MultiFOCAL's own frames module (page 6, Port 3) -
     * the exact CV+82180A configuration test/cv82180a_*_test.c already
     * verified correct. Page 4 (Port 1) is deliberately never used - see
     * nut_rom_82180a.h's own header for the confirmed-broken-for-any-
     * module finding. nut_boot() never touches pages 5/6, so this order
     * (after nut_boot()) matches every existing test's own convention
     * but doesn't actually matter for correctness. */
    sim_dbg("soynut sim: wiring HP 82180A module (page 5) and MultiFOCAL (page 6)...\n");
    nut_rom_wire_82180a();
    tabpage[6] = (short *)(const void *)rom_frames_p0;
    typmod[6] = 1;
    assert(tabpage[6] != NULL);
    assert(typmod[6] == 1);
